...
public final class RatPoly {
  ...
  private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
	if(newTerm.isZero())
		return;
    Iterator<RatTerm> iterator = lst.iterator();
	boolean done = false;
	int position = 0;
	while(iterator.hasNext() & !done) {
		RatTerm term = iterator.next();
		if(term.getExpt() == newTerm.getExpt()) {
			RatTerm sum = term.add(newTerm);
			if(sum.isZero())
				lst.remove(position);
			else
				lst.set(position, sum);
			done = true;
		} else if (term.getExpt() < newTerm.getExpt()) {
			lst.add(position, newTerm);
			done = true;
		}
		position++;
	}
  ...
  public RatPoly negate() {
    if(this.isNaN())
    	...
    	List<RatTerm> lst = new ArrayList<RatTerm>(this.terms);
  ...
  /**
   * Return the list of RatPoly class
   * @return terms
   */
  ...
  public RatPoly add(RatPoly p) {
    if(this.isNaN() || p.isNaN())
    	...
    	List<RatTerm> lst = new ArrayList<RatTerm>(this.terms);
    	for(RatTerm p_term : p.getTerms()) 
    		sortedInsert(lst, p_term);
	    ...
	    if(this.isNaN() || p.isNaN())
	    	...
	    	for(RatTerm p_term : p.getTerms()) {
	    		for(RatTerm term : this.terms)
	    			sortedInsert(lst, p_term.mul(term));
	    	}
	    	return new RatPoly(lst);
	    }  
  }
  ...
  public RatPoly div(RatPoly p) {
		...
		List<RatTerm> lst = new ArrayList<RatTerm>(terms);
		RatPoly remain = new RatPoly(lst);
		RatPoly result = new RatPoly();
		while (!remain.equals(ZERO) && remain.degree() >= p.degree()) {
			RatTerm factor = remain.getTerm(remain.degree()).div(p.getTerm(p.degree()));
			RatPoly factor_poly = new RatPoly(factor); 
			remain = remain.sub(p.mul(factor_poly));
			result = result.add(factor_poly);
		}
		result.checkRep();
		remain.checkRep();
		return result;
  ...
  public RatPoly differentiate() {
    if(this.isNaN())
    	...
    	for(RatTerm term : this.terms) {
    		derivative = derivative.add(new RatPoly(term.differentiate()));
    	}
  ...
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
    if(this.isNaN() || integrationConstant.isNaN())
    	...
    	for(RatTerm term : this.terms) {
    		antiderivative = antiderivative.add(new RatPoly(term.antiDifferentiate()));
    	}
    	antiderivative.checkRep();
    	return antiderivative.add(new RatPoly(new RatTerm(integrationConstant, 0)));
    }
  }
  ...
  public double integrate(double lowerBound, double upperBound) {
    if(this.isNaN() || lowerBound == Double.NaN || upperBound == Double.NaN) {
    	...
    	double f_upperBound = 0.0, f_lowerBound = 0.0;
    	for(RatTerm term : antiderivative.terms) {
    		f_upperBound += term.getCoeff().doubleValue() * Math.pow(upperBound, term.getExpt());
    		f_lowerBound += term.getCoeff().doubleValue() * Math.pow(lowerBound, term.getExpt());
    	}
    	return f_upperBound -  f_lowerBound;
    }
  }
