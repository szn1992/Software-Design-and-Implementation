...
public final class RatTerm {
  ...
  public RatTerm add(RatTerm arg) {
	  if(arg.isNaN() || this.isNaN()) {
		  ...
		  return arg;
	  } else if(!this.isZero() && !arg.isZero() && this.expt != arg.expt) {  
		  throw new IllegalArgumentException("exponent is not equal to argument's exponent"
											+ " (neither argument is zero or NaN)");
	  } else 
